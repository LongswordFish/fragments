...
async function readFragmentData(ownerId, id) {
  ...
  try {
    logger.debug("read data using S3 API");
...
async function deleteFragment(ownerId, id) {
  ...
  // Create a PUT Object command to send to S3
  const commandForS3 = new DeleteObjectCommand(paramsForS3);
  ...
  // Create a PUT command to send to DynamoDB
  const commandForDDB = new DeleteCommand(paramsForDDB);

  try {
    await s3Client.send(commandForS3);
  } catch (err) {
    // If anything goes wrong, log enough info that we can debug
    const { Bucket, Key } = paramsForS3;
    logger.error({ err, Bucket, Key }, 'Error deleting fragment data to S3');
  }

  try {
    await ddbDocClient.send(commandForDDB);
  } catch (err) {
    logger.warn({ err, paramsForDDB }, 'error writing fragment to DynamoDB');
    throw err;
  }
